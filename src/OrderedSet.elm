...
{-| A set of unique values that remembers their original insertion order.
...
Note: This is adapted from the OrderedSet from j-maass/elm-ordered-containers,
but with the order of the values as the _original_ insertion order. See the
example under `insert`.
...
the order state will remain the same (the original order is preserved).
